...
def add_diseases():
    ...
    cur.execute(f"""
             INSERT INTO clinical_picture(id_diseases, id_signs) VALUES ((select id_diseases from diseases 
                                    where name_diseases = '{selected_value}'),(select id_signs from signs 
                                    where name_signs = '{text}'))""")
    conn.commit()
    print_list(listbox)
...
def delete_diseases():
    ...
    if selected_indices:  # Проверяем, есть ли выбранные элементы
        for index in selected_indices:
            ...
            selected_value = combo_box.get()
            cur.execute(f"""
                         delete from clinical_picture where id_diseases =(select id_diseases from diseases where name_diseases='{selected_value}') and
                                        id_signs = (select id_signs from signs where name_signs='{selected_item}')""")
            conn.commit()
            print_list(listbox)
...
def print_list(listbox):
    ...
    selected_value = combo_box.get()
    list_value = pandas.read_sql(f""" SELECT name_signs FROM signs inner join clinical_picture using(id_signs) 
                                                    inner join diseases using(id_diseases)
                                        where name_diseases = '{selected_value}' """, conn)

    # Добавляем элементы из списка в Listbox
    for i in range(len(list_value)):
        listbox.insert(tk.END, list_value.loc[i, 'name_signs'])
...
combo_box.bind("<<ComboboxSelected>>", update_list_value)
...
list_name2 = []
my_list2 = pandas.read_sql(f""" SELECT name_signs FROM signs""", conn)
for i in range(len(my_list2)):
    list_name2.append(my_list2.loc[i, 'name_signs'])

combo_box2 = ttk.Combobox(root, values=list_name2)
# Устанавливаем текст по умолчанию
...
#надпись2
label_text = "Список признаков заболевания:"
