...
def add_diseases():
    ...
    cur.execute(f"""
             INSERT INTO possible_values(id_signs, value_pv) VALUES ((select id_signs from signs 
...
def delete_diseases():
    ...
    if selected_indices:  # Проверяем, есть ли выбранные элементы
        for index in selected_indices:
            ...
            selected_value = combo_box.get()
            cur.execute(f"""
                         delete from possible_values where value_pv ='{selected_item}' and
...
def print_list(listbox):
    ...
    selected_value = combo_box.get()
    list_value = pandas.read_sql(f""" SELECT value_pv FROM possible_values inner join signs using(id_signs) 
                                        where name_signs='{selected_value}' """, conn)

    # Добавляем элементы из списка в Listbox
    for i in range(len(list_value)):
        listbox.insert(tk.END, list_value.loc[i, 'value_pv'])
...
if list_name:
    combo_box.set(list_name[0])
else:
    combo_box.set("Добавьте признаки")
...
# надпись
label_text = "Возможное значение:"
label = tk.Label(root, text=label_text, justify="left")
label.grid(row=1, column=1, padx=10, pady=10, sticky="sw")

# поле для ввода названий
entry = tk.Entry(root, width=30)
entry.grid(row=1, column=1, padx=10, pady=10, sticky="e")
...
# надпись2
label_text = "Список возможных значений:"
