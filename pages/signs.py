...
def add_diseases():
    text = entry.get()
    cur.execute(f"""
             INSERT INTO signs(name_signs) VALUES ('{text}')""")
    conn.commit()
    entry.delete(0, "end")
    print_list(listbox)
...
def delete_diseases():
    ...
    if selected_indices:  # Проверяем, есть ли выбранные элементы
        for index in selected_indices:
            ...
            cur.execute(f"""
                         delete from signs where name_signs ='{selected_item}'""")
            conn.commit()
            print_list(listbox)
...
def print_list(listbox):
    ...
    my_list = pandas.read_sql(""" SELECT name_signs FROM signs """, conn)

    # Добавляем элементы из списка в Listbox
    for i in range(len(my_list)):
        listbox.insert(tk.END, my_list.loc[i, 'name_signs'])
